...
public class RightHemisphere extends CustomRelic {
    // ID, images, text.
    public static final String ID = SlayByDay.makeID("RightHemisphere");
...
//    private static String power_just_added = "";
    ...
    public void update() {
        super.update();
        if (AbstractDungeon.player.hasRelic(LeftHemisphere.ID)) {
            AbstractDungeon.player.relics.add(new UnitedMind());
            AbstractDungeon.player.loseRelic(LeftHemisphere.ID);
